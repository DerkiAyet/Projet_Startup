...
async function discoverContentService() {
  try {
    const instances = eurekaClient.getInstancesByAppId(process.env.CONTENT_SERVICE_NAME);

    if (!instances || instances.length === 0) {
      console.warn("Aucune instance CONTENT-SERVICE disponible via Eureka, utilisation de la configuration par défaut");
      return process.env.CONTENT_SERVICE_URL;
    }

    const instance = instances[Math.floor(Math.random() * instances.length)];
    const baseUrl = `http://${instance.hostName}:${instance.port["$"] || 8000}`;
    console.log(`Utilisation de l'instance CONTENT-SERVICE: ${baseUrl}`);
    return baseUrl;
  } catch (error) {
    console.error("Erreur de découverte de service:", error);
    return process.env.CONTENT_SERVICE_URL;
...
module.exports = { discoverAuthService, discoverContentService };