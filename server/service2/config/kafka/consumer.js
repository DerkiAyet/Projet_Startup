...
const consumer = kafka.consumer({ groupId: 'posts-service-group-v2' });
    ...
    await consumer.run({
        eachMessage: async ({ topic, message }) => {
            ...
            if (topic === 'student-interests.sync') {
                studentInterestsCache.set(
                    String(payload.data.idStudent),
                    payload.data.interestIds        // ← stocke le tableau directement
                );
            }

            if (topic === 'teacher-expertise.sync') {
                teacherExpertiseCache.set(
                    String(payload.data.idTeacher),
                    payload.data.expertiseIds
                );
            }
        }
    });
...
// Getters avec fallback HTTP si pas encore en cache
